...
- \`leads\`: every prospect contact. Key fields: \`status\` (new_lead | qualified | contacted | follow_up | nurture | won | lost), \`outreach.linkedIn.{status, sentAt, repliedAt, openedAt, refusedAt, noResponseAt}\`, \`outreach.email.{...same}\`, \`companyId\`, \`companyIndustry\`, \`archived\`, \`updatedAt\`, \`createdAt\`. OutreachStatus enum: not_sent | sent | opened | replied | refused | no_response.
- \`companies\`: the company-level rollup. Key fields: \`name\`, \`industry\`, \`status\` (same enum as leads, auto-synced majority), \`statusLockedManually\`, \`statusLastUpdatedAt\`, \`apolloEnrichment\`, \`blogAnalysis\`.
- \`nikolaWorkQueue\`: every job Nikola processed. Key fields: \`kind\` (try | enrich | find-leads | find-companies | mention | status | analytical-query | multi-step | remember), \`status\` (pending | processing | completed | failed), \`createdAt\`, \`source\` (slash | mention).
