// src/components/features/companies/ProgramUrlFieldMappingSelector.tsx
...
import {
  ...
  saveProgramUrlFieldMapping,
  clearProgramUrlFieldMapping,
  getPotentialProgramUrlFields,
} from '../../../services/api/programUrlFieldMappingService';

interface ProgramUrlFieldMappingSelectorProps {
  companies: Company[];
}

export const ProgramUrlFieldMappingSelector: React.FC<ProgramUrlFieldMappingSelectorProps> = ({
  companies,
}) => {
  ...
  useEffect(() => {
    const savedMapping = getProgramUrlFieldMapping();
    if (savedMapping) {
      setSelectedField(savedMapping);
    }

    const fields = getPotentialProgramUrlFields(companies);
  ...
  const handleFieldChange = (fieldName: string) => {
    if (fieldName === '') {
      ...
      // Save new mapping
      saveProgramUrlFieldMapping(fieldName);
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      <Tooltip title="Map a custom field to automatically use existing program URLs during analysis">
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          <LinkIcon sx={{ fontSize: 18, color: '#667eea' }} />
